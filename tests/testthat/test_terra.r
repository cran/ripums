...
test_that("Terra raster works", {
  ...
  if (!file.exists(raster_file)) {
    skip("Couldn't find raster example file. ripumsexamples likely not installed.")
  }
  skip_if_not_installed("raster")
  raster <- ripums:::read_terra_raster_list(raster_file, verbose = FALSE)
...
test_that("Terra micro works", {
  ...
  if (!file.exists(micro_file)) {
    skip("Couldn't find micro example file. ripumsexamples likely not installed.")
  }
  skip_if_not_installed("sf")

  micro <- ripums:::read_terra_micro_sf(micro_file, verbose = FALSE)
...
test_that("Terra area works", {
  ...
  if (!file.exists(area_file)) {
    skip("Couldn't find area example file. ripumsexamples likely not installed.")
  }
  skip_if_not_installed("sf")

  area <- ripums:::read_terra_area_sf(area_file, verbose = FALSE)
